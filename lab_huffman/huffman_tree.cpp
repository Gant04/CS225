...
{
		TreeNode* temp=NULL;//= singleQueue.peek();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    queue<TreeNode*> mergeQueue;  // Queue containing the inner nodes
		...
		//	std::cout<<frequencies[i].getCharacter()<<" "<<frequencies[i].getFrequency()<<std::endl;
		...
		int new_freq=smallest1->freq.getFrequency()+smallest2->freq.getFrequency();
		//char new_character=smallest1->freq.getCharacter()+smallest2->freq.getCharacter();

		//Frequency freqer(new_character,new_freq);

		TreeNode* new_tree= new TreeNode(new_freq);

		new_tree->left=smallest1;
		new_tree->right=smallest2;

		mergeQueue.push(new_tree);
		}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     while(bfile.hasBits()){
        if(bfile.getNextBit()){
          TreeNode* new_tree= new TreeNode(Frequency(bfile.getNextByte(),0));
          return new_tree;
        } else{
            TreeNode* new_tree= new TreeNode(0);
            new_tree->left=readTree(bfile);
            new_tree->right=readTree(bfile);
            return new_tree;
        }
     }
