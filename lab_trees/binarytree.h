...
class BinaryTree
{
	...
	T max(Node* root) const;

	void isOrderedRecursive(Node* root_,std::vector<T>& inorder_vector) const;
	...
	void getPaths(std::vector<std::vector<T>>& paths,std::vector<T> path,Node* root) const;
         ...
         */
				...
				int sumDistances(int distance, Node* root_)const;
