...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
	...
	while(current != "p1-0" && current !="p2-0"){
		int choice= rand()%2;
		Vertex old_current=current;
		int num=std::stoi(current.substr(3,current.length()));
		if(current[1]=='1'){
			 if(num==1){
					current="p2-0";
			 }else{
					if(choice==0){
							current="p2-"+std::to_string(num-1);
					} else if(choice==1){
							current="p2-"+std::to_string(num-2);
					}
				}
		} else if(current[1]=='2'){
			 if(num==1){
					...
					if(choice==0){
						current="p1-"+std::to_string(num-1);
					} else if(choice==1){
						current="p1-"+std::to_string(num-2);
					}
				}
		}
		path.push_back(g_.getEdge(old_current,current));
