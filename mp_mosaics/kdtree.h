...
class KDTree
{
  ...
  public:
     ...
     */
		...
		int Partition(vector<Point<Dim>> & newPoints,int left,int right,int currDim);

		int QuickSelect(vector<Point<Dim>> & newPoints,int left,int right,int k,int currDim);
		
	  	private:
		double radius(const Point<Dim> first,const Point<Dim> second) const;
		KDTreeNode* KDTree_helper(vector<Point<Dim>> & newPoints,int left,int right,int currDim);
